...
//            return null;
    ...
    public PageInfo<DeliverDetail> getCustomerListBySales(Integer dealerId, Integer pageNum, Integer pageSize,
        ...
        List<DeliverDetail> result = deliverDetailMapper.getCustomerListBySales(dealerId,
                uploadStartTime, uploadEndTime, handoverStartTime, handoverEndTime,
                customerFullName, productModel, deliveryType, orderMonth, customerOrderNumber, user.getUserName());
        return new PageInfo<>(result);
    }
...
//        FileVO fileVo = FileUtil.upload(excel, deliverLocalPath);
    ...
    public void uploadDataByUpdate(MultipartFile excel, Integer userId) {
        ...
        List<DeliverDetailUpdate> oldReocrd = new ArrayList<>();
        ...
        for(DeliverDetailUpdate detail : oldReocrd) {
            deliverDetailUpdateMapper.insertSelective(detail);
        }
