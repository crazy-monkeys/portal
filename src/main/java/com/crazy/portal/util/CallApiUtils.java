...
public class CallApiUtils {
    ...
    public static Map<String,String> callC4cCustomerInfo(CustomerInfoCreate create) {
        ...
        try {
            String requestXml = JaxbXmlUtil.convertToXml(create).replace("</yor:CorporateAssets currencyCode=\"CNY\">", "</yor:CorporateAssets>");
            log.info(requestXml);
            String response = HttpClientUtils.post(url, requestXml);
            log.info(response);
    ...
    public static void callC4cCustomerDetail(CustomerDetailCreate create) {
        ...
        try {
            String requestXml = JaxbXmlUtil.convertToXml(create).replace("</Revenue currencyCode=\"USD\">", "</Revenue>").replace("</NetAssets currencyCode=\"USD\">","</NetAssets>").replace("</TotalAssets currencyCode=\"USD\">","</TotalAssets>");
            log.info(requestXml);
            String response = HttpClientUtils.post(url, requestXml);
            log.info(response);
